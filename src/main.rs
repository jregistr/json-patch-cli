...
fn main() -> Result<()> {
    ...
    let json = to_value(doc).context("Could not read the Doc input file")?;
    let patch = to_value(patch).context("Failed to read the Patch input file")?;

    let mut doc =
        serde_json::from_str::<Value>(json.as_str()).context("Could not parse the Doc as JSON")?;

    let patch = serde_json::from_str::<json_patch::Patch>(patch.as_str())
        .context("Could not parse the Patch as JSON")?;

    json_patch::patch(&mut doc, &patch).context("Applying the patch to the JSON doc failed.")?;
